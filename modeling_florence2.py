    ...
    def forward(
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(
                #input_ids=input_ids,  #We don't need to pass this to encoder as it is getting inputs_embeds
                attention_mask=attention_mask,
                head_mask=head_mask,
...
class Florence2LanguageForConditionalGeneration(Florence2LanguagePreTrainedModel):
    ...
    def forward(
        ...
        """
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        print("labels from F2L for CG", labels)
        ...
        print("Decoder from F@L for CG", )
        print("input_ids from F2L for CG", input_ids)
        outputs = self.model(
            input_ids,
            attention_mask=attention_mask,
