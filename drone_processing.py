...
class Florence2Processor(ProcessorMixin):
    ...
    def text(
        ...
        text = f"""You are a Drone your Task is "{text}" . Based on current frame move Left or Right, Up or Down, Forward or backward, Rotate Left or Rotate Right"""
        ...
        if padding:
            ...
            if padding_length > 0:
                pad_token_id = 1
                inputs['input_ids'] = torch.cat([inputs['input_ids'], torch.full((inputs['input_ids'].shape[0], padding_length), pad_token_id)], dim=1)
                # Adjust attention mask to ignore padded tokens
                inputs['attention_mask'] = torch.cat([inputs['attention_mask'], torch.zeros((inputs['attention_mask'].shape[0], padding_length), dtype=torch.long)], dim=1)
